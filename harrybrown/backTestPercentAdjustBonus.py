...
# 初始净值归一化和初始权重
initial_weights = {'沪深300': 0.25, '黄金': 0.25, '国债': 0.35, '现金': 0.15}
...
def rebalance_portfolio_if_needed(row, weights):
    ...
    # 检查是否触发再平衡条件
    if (weights_dynamic['沪深300'] > 0.35 or weights_dynamic['沪深300'] < 0.15 or
        weights_dynamic['黄金'] > 0.35 or weights_dynamic['黄金'] < 0.15 or
        weights_dynamic['国债'] > 0.45 or weights_dynamic['国债'] < 0.25 or
        weights_dynamic['现金'] > 0.25 or weights_dynamic['现金'] < 0.05):
        return sum(row[asset] * initial_weights[asset] for asset in initial_weights), initial_weights.copy()

    return total_value, weights_dynamic
