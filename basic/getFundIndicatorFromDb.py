...
def calculate_annualized_return(df, years=None):
    """
    计算年化收益率。
    如果指定 years 参数，则只计算近 years 年的数据；否则计算全部数据。
    """
    if df.empty:  # 检查 DataFrame 是否为空
        return None
    ...
    if years:
        period_years = years  # 如果传入了特定年数，使用它
    else:
...
def calculate_and_save_results(fund_codes):
    ...
    for fund_code in fund_codes:
        try:
            ...
            errors.append({'基金代码': fund_code, '错误信息': str(e)})

    # 将结果存储到 DataFrame
    df_results = pd.DataFrame(results)
    df_results.to_excel('./data/fund_performance.xlsx', index=False)
...
def main():
    ...
    # calculate_and_save_results("000459")  # 计算并保存结果
